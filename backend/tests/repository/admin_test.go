...
func setupAdminTestDB(t *testing.T) *gorm.DB {
	...
	// Auto-migrate the schema for the models we will be testing against
...
func TestAdminRepositoryImpl_CountUsers(t *testing.T) {
	...
	// Since we are in a different package, we must use the exported constructor
	repo := repository.NewAdminRepository(db)

	// Case 1: No users in the database
	count, err := repo.CountUsers()
	assert.NoError(t, err)
	assert.Equal(t, int64(0), count)

	// Case 2: Seed the database with some users
...
func TestAdminRepositoryImpl_CountProblems(t *testing.T) {
	...
	// Case 1: No problems in the database
	count, err := repo.CountProblems()
	assert.NoError(t, err)
	assert.Equal(t, int64(0), count)

	// Case 2: Seed the database with some problems
...
func TestAdminRepositoryImpl_CountSubmissions(t *testing.T) {
	...
	// Case 1: No submissions in the database
	count, err := repo.CountSubmissions()
	assert.NoError(t, err)
	assert.Equal(t, int64(0), count)

	// Case 2: Seed the database with some submissions
